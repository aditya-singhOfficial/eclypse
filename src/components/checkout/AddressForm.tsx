...
const addressSchema = z.object({
  ...
  zip: z.string().min(4, "Please enter a valid postal code").max(10, "Postal code is too long")
    .refine(val => /^[0-9]{4,6}$/.test(val), { message: "Postal code must be 4-6 digits" }),
});
    ...
    <form onSubmit={handleSubmit(handleFormSubmit)} className="w-full max-w-2xl bg-gray-900 border border-gray-800 p-6 flex flex-col gap-5 mx-auto">
      ...
      <div className="flex flex-col md:flex-row gap-4">
        ...
        <div className="flex-1">
          ...
          {errors.zip && <span className={errorClasses}>{errors.zip.message}</span>}
        </div>
      </div>
